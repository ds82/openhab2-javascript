...
var logger = Java.type('org.slf4j.LoggerFactory').getLogger(
  'org.eclipse.smarthome.automation.module.script.rulesupport.internal.shared.SimpleRule'
);
var uuid = Java.type('java.util.UUID');
var ScriptExecution = Java.type(
  'org.eclipse.smarthome.model.script.actions.ScriptExecution'
);
var ScriptServiceUtil = Java.type(
  'org.eclipse.smarthome.model.script.ScriptServiceUtil'
);
...
var RewindFastforwardType = Java.type(
  'org.eclipse.smarthome.core.library.types.RewindFastforwardType'
);
var PlayPauseType = Java.type(
  'org.eclipse.smarthome.core.library.types.PlayPauseType'
);
var NextPreviousType = Java.type(
  'org.eclipse.smarthome.core.library.types.NextPreviousType'
);
var DateTimeType = Java.type(
...
require('lib/persistenceExtensions.js');
...
(function(context) {
  ...
  context.console.log = function(value) {
    var log = Array.isArray(value) ? value : [value];
  ...
  context.isUndefinedStr = function(itemStr) {
    return ir.getItem(itemStr)
      ? isUndefinedState(ir.getItem(itemStr).state)
      : true;
  };

  context.isUndefinedState = function(itemState) {
    if (
      itemState.toString() == 'Uninitialized' ||
      itemState.toString() == 'Undefined'
    )
      return true;
    return false;
  };
  ...
  context.isUninitialized = function(it) {
    try {
      ...
      if (
        item == null ||
        item.state instanceof UnDefType ||
        item.state.toString() == 'Undefined' ||
        item.state.toString() == 'Uninitialized'
      )
        return true;
    } catch (err) {
  ...
  context.setTimeout = function(fn, ms) {
    ms = ms || 1;
    var time = DateTime.now().plusMillis(ms);
    return ScriptExecution.createTimer(time, fn);
  };

  //round(ungerundeter Wert, Stellen nach dem Komma); round(6,66666, 2); -> 6,67
  context.round = function(x, p) {
