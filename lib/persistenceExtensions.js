 ...
 * https://github.com/lewie/openhab2-javascript/blob/master/jslib/PersistenceExtensions.js
 */
...
(function(context) {
  ...
  context.persist = function(it, serviceId) {
    try {
      var item = context.getItem(it);
      serviceId == undefined
        ? context.pe.persist(item)
        : context.pe.persist(item, serviceId);
    } catch (err) {
  ...
  context.historicState = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.historicState(item, timestamp).state
        : context.pe.historicState(item, timestamp, serviceId).state;
    } catch (err) {
  ...
  context.changedSince = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.changedSince(item, timestamp)
        : context.pe.changedSince(item, timestamp, serviceId);
    } catch (err) {
  ...
  context.maximumSince = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.maximumSince(item, timestamp).state
        : context.pe.maximumSince(item, timestamp, serviceId).state;
    } catch (err) {
  ...
  context.minimumSince = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.minimumSince(item, timestamp).state
        : context.pe.minimumSince(item, timestamp, serviceId).state;
    } catch (err) {
  ...
  context.averageSince = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.averageSince(item, timestamp)
        : context.pe.averageSince(item, timestamp, serviceId);
    } catch (err) {
  ...
  context.sumSince = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.sumSince(item, timestamp)
        : context.pe.sumSince(item, timestamp, serviceId);
    } catch (err) {
  ...
  context.lastUpdate = function(it, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.lastUpdate(item)
        : context.pe.lastUpdate(item, serviceId);
    } catch (err) {
  ...
  context.deltaSince = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.deltaSince(item, timestamp)
        : context.pe.deltaSince(item, timestamp, serviceId);
    } catch (err) {
  ...
  context.evolutionRate = function(it, timestamp, serviceId) {
    try {
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.evolutionRate(item, timestamp)
        : context.pe.evolutionRate(item, timestamp, serviceId);
    } catch (err) {
  ...
  context.previousState = function(it, skipEqual, serviceId) {
    try {
      ...
      var item = context.getItem(it);
      return serviceId == undefined
        ? context.pe.previousState(item, skipEqual).state
        : context.pe.previousState(item, skipEqual, serviceId).state;
    } catch (err) {
